...
<br/><br/><br/>	
	<h2 align="center">독후감</h2>
	<div align="center">
      <table border="1" style="height:200px; width: 600px;">
    	<tr>
    		<th>번호</th>
    		<td>${bookReviewData.bookReview.bNo}</td>
    	</tr>
    	<tr>
    		<th>책제목</th>
    		<td>${bookReviewData.bookReview.bookTitle}</td>
    	</tr>
      	<tr>
    		<th>저자</th>
    		<td>${bookReviewData.bookReview.author}</td>
    	</tr>
       	<tr>
    		<th>출판사</th>
    		<td>${bookReviewData.bookReview.publisher}</td>
    	</tr>
    	
    	
    	<tr>
    		<th>작성자명</th>
    		<td>${bookReviewData.bookReview.bWriterName}</td>
    	</tr>
    	<tr>
    		<th>제목</th>
    		<td>${bookReviewData.bookReview.bTitle}</td>
    	</tr>
    	<tr>
    		<th>내용</th>
    		<td>${bookReviewData.bookReview.bContent}</td>
    	</tr>
    	<tr>
    		<th>작성일</th>
    		<td><fmt:formatDate pattern="yyyy년 MM월 dd일" value="${bookReviewData.bookReview.regDate}"/></td>
    	</tr>
    </table>
    	...
    	<c:if test="${! empty authUser && (authUser.grade!=999)}">
	    <a href="<%=request.getContextPath()%>/bupdate.do?no=${bookReviewData.bookReview.bNo}&pageNo=${pageNo}&rowSize=${rowSize}">글 수정</a>
	    <a href="<%=request.getContextPath()%>/bdelete.do?no=${bookReviewData.bookReview.bNo}">글 삭제</a>
	    </c:if>
	    <c:if test="${! empty authUser && (authUser.grade==999)}">
	    <a href="<%=request.getContextPath()%>/bupdate.do?no=${bookReviewData.bookReview.bNo}&pageNo=${pageNo}&rowSize=${rowSize}">글 수정</a>
	    <a href="<%=request.getContextPath()%>/bdelete.do?no=${bookReviewData.bookReview.bNo}">글 삭제</a>
