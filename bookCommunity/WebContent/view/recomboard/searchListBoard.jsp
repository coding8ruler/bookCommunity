	   		...
	   		<tr>
	    		<td colspan="6" style="text-align: center;">
	    			...
	    			<c:if test="${boardPage.startPage > 5}">
	    				<a href="/recomboardSearch.do?pageNo=${boardPage.startPage-5}&rowSize=${rowSize}&word=${word}&col=${col}">prev</a>
	    			</c:if>
	    			
	    			<%-- JSTL forEach조건문 : 페이지번호출력 --%>
	    			<c:forEach var="pNo" begin="${boardPage.startPage}" end="${boardPage.endPage}">
	    				<a href="/recomboardSearch.do?pageNo=${pNo}&rowSize=${rowSize}&word=${word}&col=${col}">${pNo}</a>
	    			</c:forEach>
	    			
	    			<%-- JSTL if조건문 : 다음출력 --%>
	    			<c:if test="${boardPage.endPage < boardPage.totalPages}">
	    				<a href="/recomboardSearch.do?pageNo=${boardPage.startPage+5}&rowSize=${rowSize}&word=${word}&col=${col}">next</a>
	    			</c:if>
	    		</td>
	    	</tr>
    	</tbody>
