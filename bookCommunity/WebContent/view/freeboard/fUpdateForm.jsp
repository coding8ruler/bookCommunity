...
<br/><br/><br/><br/><br/>
 ...
 <h2>자유게시글 수정</h2>
 <hr/>
 <form name="updateFrm" id="updateFrm" method="post" action="<%=request.getContextPath()%>/fUpdate.do">
 	  <input type="hidden" name="no" id="no" value="${upReq.no}"/>
 	  <input type="hidden" name="writerName" id="writerName" value="${upReq.writerName}"/>	  
 <div class="form-group">
 <label for="writer_name">작성자</label>
		<input type="text" name="writername" class="form-control" value="${upReq.writerName}">
 </div>
 <div class="form-group">
		<label for="title">제목</label>
		<input type="text" name="fTitle" id="fTitle" class="form-control" value="${upReq.fTitle}">
</div>
<div class="form-group">
		 	<textarea name="fContent" id="fContent" rows="20" cols="100">${upReq.fContent}</textarea>
		 </div>
	<script src="https://cdn.ckeditor.com/ckeditor5/36.0.0/classic/ckeditor.js"></script>
	<script>
      ClassicEditor.create( document.querySelector( '#fContent' ) );
      language: "ko"
    </script>
		<button onClick="submit" class="btn btn-primary btn-sm btnReplyU">수정</button>
		 <button onClick="submit"  class="btn btn-secondary btn-sm btnReplyN" onclick="location.href='<%=request.getContextPath()%>/fList.do?rowSize=${rowSize}'";>취소</button>
		 <button onClick="submit"  class="btn btn-warning btn-sm btnReplyD" onclick="location.href='<%=request.getContextPath()%>/fDelete.do?no=${upReq.no}'";>삭제</button>
</form>
</div>	
</body>
</html>
