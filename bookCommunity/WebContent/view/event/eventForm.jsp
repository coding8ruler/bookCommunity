...
$(window).scroll(function(){  //스크롤이 움직일때마다 이벤트 발생
	...
	$(function() {
		$("#btnWrite").click(function() {
			location.href="<%=request.getContextPath()%>/eWrite.do?rowSize=${rowSize}";
		});
	});
...
<body>
	...
	<br/>
	<div id="formDiv">
	<form name="rowSizeFrm" id="rowSizeFrm" action="<%=request.getContextPath()%>/event.do" method="get">
        게시물수 :
    ...
    <table border="1" style="width: 1200px;">
    	...
    	<tbody>
	    	...
	    	<tr> 
	   			...
	   			<td><a href="<%=request.getContextPath()%>/eRead.do?no=${event.e_no}&pageNo=${eventPage.currentPage}&rowSize=${size}">${event.e_title}</td>
	   			<td>
			        <fmt:formatDate pattern="yyyy년 MM월 dd일" value="${event.regdate}" /><br/>
	   			</td>
	   			<td>${event.e_cnt}</td>
	   		</tr>	
	    	</c:forEach>
	   		...
	   		<tr>
	    		<td colspan="5" style="text-align: center;">
	    			<c:if test="${eventPage.startPage > 5}">
	    				<a href="<%=request.getContextPath()%>/event.do?pageNo=${eventPage.startPage-5}&rowSize=${size}">prev</a>
	    			</c:if>
	    			<c:forEach var="pNo" begin="${eventPage.startPage}" end="${eventPage.endPage}">
	    				<a href="<%=request.getContextPath()%>/event.do?pageNo=${pNo}&rowSize=${size}">${pNo}</a>
	    			</c:forEach>
	    			<c:if test="${eventPage.endPage < noticePage.totalPages}">
	    				<a href="<%=request.getContextPath()%>/event.do?pageNo=${eventPage.startPage+5}&rowSize=${size}">next</a>
	    			</c:if>
	    		</td>
	    	</tr>
    	</tbody>
